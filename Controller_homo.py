...
#        return stlen
    ...
    def checkForViability(self, path, isBest, src, dst):
        if self.ongoingVideos == None:
            return False
        if self.ongoingVideos.get(src) == None:
            return False
        if self.ongoingVideos.get(src).get(dst) == None:
...
class BQoEPathController(ControllerBase):
    ...
    @route('bqoepath', url, methods=['GET'], requirements={'method': r'listpaths-h[0-9]+-h[0-9]+'})
