...
def calculate_composed_mos(self, path):
    ...
    for i in range(0, (len(path) - 1)):
        index2 = self.nodes.index(path[i + 1])
        index1 = self.nodes.index(path[i])
        final_bandwidth = self.bw[index1][index2] if (self.bw[index1][index2] < final_bandwidth) else final_bandwidth
    ...
    while not q.empty():
        ...
        for neighbour in indarr:
            if self.links[neighbour][node] == 1:
                ...
                # return dict(mos = -1, tp = -1, dst = "", dest_ip="", path = "OK")
                # mos = round(mos, 2)
                if (self.calculateDistanceMetric(mos, bwDistanceAux) < (self.calculateDistanceMetric(mosNode[neighbour],
                                                                                                     bwDistanceNode[
